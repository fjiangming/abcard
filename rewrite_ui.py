...
tab_run, tab_accounts, tab_history, tab_sync = st.tabs(["执行", "账号", "历史", "同步"])
...
with tab_run:
    ...
    if st.session_state.running:
        ...
        # 实时日志: 每次 rerun 拉取最新日志并展示
        pull_captured_logs()
        _live_logs = st.session_state.get("log_buffer", [])
        if _live_logs:
    ...
    if st.session_state.result and not st.session_state.running:
        ...
        if st.session_state.log_buffer:
            ...
            import streamlit.components.v1 as _components
            import html as _html_mod
            _log_text = "\n".join(st.session_state.log_buffer[-200:])
            _log_escaped = _html_mod.escape(_log_text)
            _components.html(f"""
                <style>
                    body {{ text-size-adjust: 100%; -webkit-text-size-adjust: 100%; margin: 0; padding: 0; }}
            ...
            if _fn.startswith("credentials_") and _fn.endswith(".json"):
                ...
                try:
                    with open(_fpath, encoding="utf-8") as _f:
                        _export_files.append((_fpath, json.load(_f)))
                except Exception:
                    pass
    ...
    with _exp_col1:
        if _export_files:
            ...
            with zipfile.ZipFile(_zip_buf, "w", zipfile.ZIP_DEFLATED) as _zf:
                _seen_emails = {}
                for _fpath, _cd in _export_files:
                    ...
                    _zf.writestr(_arc_name, json.dumps(_cd, ensure_ascii=False, indent=2))
            _zip_buf.seek(0)
            from datetime import datetime as _dt
            _zip_name = f"credentials_{_dt.now().strftime('%Y%m%d_%H%M%S')}.zip"
            st.download_button(
                f"📦 导出全部 ({len(_export_files)} 个账号)",
...
with tab_sync:
    ...
    # ── 配置区域 ──
    with st.expander("NewAPI 配置", expanded=True):
    ...
    if not _cred_files:
        ...
        if _cred_data_list:
            import pandas as pd
            _sync_rows = []
            for _cd in _cred_data_list:
                _sync_rows.append({
                    "文件": _cd["_filename"],
                    "邮箱": _cd.get("email", "-"),
                    "refresh_token": "✅" if _cd.get("refresh_token") else "❌",
                    "access_token": "✅" if _cd.get("access_token") else "❌",
                })
            st.dataframe(pd.DataFrame(_sync_rows), hide_index=True, use_container_width=True)
            st.caption(f"共 {len(_cred_data_list)} 个 credentials 文件")

            # ── 导入按钮 ──
            _sync_col1, _sync_col2 = st.columns([3, 1])
            with _sync_col1:
                _sync_all_btn = st.button(
                    f"全部导入 ({len(_cred_data_list)} 个)",
                    type="primary", use_container_width=True, key="sync_all_btn",
                    disabled=not _newapi_base or not _newapi_token,
                )
            with _sync_col2:
            ...
            def _do_newapi_import(cred_list, base_url, admin_token, ch_type, models, group, priority, weight):
                ...
                for cd in cred_list:
                    ...
                    # 构建 key: 复制一份不含内部字段的 dict
                    key_data = {k: v for k, v in cd.items() if not k.startswith("_")}
            ...
            if _sync_all_btn:
                with st.spinner(f"正在导入 {len(_cred_data_list)} 个账号..."):
                    _import_results = _do_newapi_import(
                        _cred_data_list, _newapi_base, _newapi_token,
            ...
            for _idx, _cd in enumerate(_cred_data_list):
                _email = _cd.get("email", "unknown")
                _single_col1, _single_col2 = st.columns([4, 1])
                with _single_col1:
                    _has_rt = "🔑" if _cd.get("refresh_token") else ""
                    st.text(f"{_email} {_has_rt}")
                with _single_col2:
                    if st.button("导入", key=f"sync_single_{_idx}",
                                 disabled=not _newapi_base or not _newapi_token):
